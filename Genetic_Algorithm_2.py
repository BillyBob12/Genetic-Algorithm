...
def main_2():
	...
	file_best_Fit=open('20180417.txt','w',encoding='UTF-8')
	Fit=[]
	Fitness(Fit,chromosome,populationsize)
	for i in range(iterations):   #代数
		Fitcopy=[]
		# print('Fit',Fit)
		Sort(Fit,chromosome)
		...
		# 	print(chromosome[i])
		Chromosomecopy=[[]]
		Top_n_persent(Fit,Topn, Chromosomecopy,chromosome)  #种群的前Topn直接进入下一代
		for i in range(int(len(Fit)*Topn)):
			Fitcopy.append(Fit[i])
		# print('Chromosomecopy', Chromosomecopy)
		for i in range(int((populationsize-int(populationsize*Topn))/2)):  #剩余的populationsize-populationsize*Topn
																			# 采用轮盘赌方式选择父代母代进行交叉遗传得到
			p2=[]
			p1=[]
			selections(Fit,p1,chromosome)
